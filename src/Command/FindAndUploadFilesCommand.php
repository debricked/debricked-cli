...
class FindAndUploadFilesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($finder as $file) {
            ...
            if (\in_array($fileName, $dependencyFileNames['dependencyFileNames']) === true) {
                if (\in_array($fileName, $dependencyFileNames['dependencyFileNamesRequiresAllFiles']) === true) {
                    // If we find an adjacent dependency tree file already generated by the customer, use it, otherwise warn.
                    $adjacent = $this->getAdjacentDependencyTreeFile($file, $dependencyFileNames['adjacentDependencyFileNames']);
                    if (!empty($adjacent) && empty($uploadAllFiles) === true) {
                        // There is an adjacent file! Add it to the zip. The adjacent file is, by definition, adjacent
                        // to the current $file, so we can use $file->getRelativePath too.
                        $adjacentRelativePath = $file->getRelativePath().'/'.$adjacent;
                        $zip->addFile($adjacent, $adjacentRelativePath);
                        $uploadedAdjacentFilePaths[] = $adjacentRelativePath;
                    } elseif (empty($uploadAllFiles) === true) {
     ...
     *
     * @return string|null List to the full path of the adjacent file, if it exists, otherwise null
     */
    private function getAdjacentDependencyTreeFile(\SplFileInfo $file, array $adjacentDependencyFileNames): ?string
    {
        $filename = $file->getFilename();
